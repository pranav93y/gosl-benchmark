...
llc_miss = list(columns.values())[5]
insn = list(columns.values())[1]

for x, y in zip(llc_miss, insn):
    val = (float(x)/float(y))*1000
    values.append(val)


plt.xlabel('Time',  fontsize=18)
plt.ylabel('LLC miss per Kilo Insn',  fontsize=18)
plt.title('LLC miss per Kilo Insn vs Time',  fontsize=20)
plt.grid(True)
plt.plot(timestamps, values, 'r-', label="LLC miss per Kilo Insn")
leg = plt.legend()
