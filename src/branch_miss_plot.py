...
branch_miss = list(columns.values())[3]
insn = list(columns.values())[1]

for x, y in zip(branch_miss, insn):
    val = (float(x)/float(y))*1000
    values.append(val)


plt.xlabel('Time',  fontsize=18)
plt.ylabel('Branch miss per Kilo Insn',  fontsize=18)
plt.title('Branch miss per Kilo Insn vs Time',  fontsize=20)
plt.grid(True)
plt.plot(timestamps, values, 'g-', label="Branch miss per Kilo Insn")
leg = plt.legend()
